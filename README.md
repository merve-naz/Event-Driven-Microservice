...
The project uses a centralized logging configuration provided by the **app-config-data** module.

Each microservice includes the shared `logback-common.xml` configuration and only defines its own application name and log directory.
...
All log messages generated by the application are automatically processed according to the shared Logback configuration and written to both the console and the corresponding log file.
